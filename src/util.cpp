...
const char * const BITCOIN_CONF_FILENAME = "greenfrogcoin.conf";
const char * const BITCOIN_PID_FILENAME = "greenfrogcoind.pid";
...
#else
    const char* pszModule = "greenfrogcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Greenfrogcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Greenfrogcoin";
#else
    // Unix
    return pathRet / ".greenfrogcoin";
#endif
#endif
}
